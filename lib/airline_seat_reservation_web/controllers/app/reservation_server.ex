defmodule App.ReservationServer do

    def start(name, paxos_proc) do
        pid = spawn(ReservationServer, :init, [name, paxos_proc])
        IO.puts("#{inspect name} #{inspect paxos_proc}")
    ...
    def testing_server() do
        "Sever is reached"
    end

    def deposit(r, amount) do # This function is invoked when the user first says deposite where r = pid of one of the ReservationServer
    ...
    def run(state) do
        state = receive do
            {trans, client, _}=t when trans == :deposit or trans == :withdraw ->
                ...
                if Paxos.propose(state.pax_pid, state.last_instance+1, t, 1000) == {:abort} do #value of t is a log step for exampel, it could be {:deposite, client, 10}
                    send(client, {:abort})
                else
